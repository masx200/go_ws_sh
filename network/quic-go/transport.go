...
func (t *transport) serveConn(tlsConf *tls.Config, conn net.PacketConn) error {
	...
	if conn == nil {
		...
		lnp, err = (quic.ListenAddrEarly(addr, baseConf, quicConf))

	} else {
		lnp, err = quic.ListenEarly(conn, baseConf, quicConf)
